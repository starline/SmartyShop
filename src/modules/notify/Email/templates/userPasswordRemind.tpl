...
<body>
	<p>{$user->name|escape}, на сайте <a href='{$config->root_url}'>{$settings->site_name}</a> был сделан запрос
		на восстановление вашего пароля.</p>
	<p>Вы можете изменить пароль, перейдя по следующей ссылке:</p>
	<p><a href='{$config->root_url}/user/password-remind/{$code}'>Изменить пароль</a></p>
