...
{if !$payment_method->public_name|empty}<b>Оплата:</b> {$payment_method->public_name|escape}{PHP_EOL}{PHP_EOL}{/if}
К оплате: <b>{$order->payment_price}</b> {$currency->sign}
{$url = "{$config->root_url}/order/{$order->url}"}{$url_text = "Открыть заказ на сайте"}