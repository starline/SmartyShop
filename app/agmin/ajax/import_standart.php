...
class ImportAjax extends Auth
{
    ...
    private $import_file           = 'standart_import.csv';    	// Временный файл
    ...
    private function import_item($item)
    {
        ...
        if (!empty($item['category'])) {
            ...
            // Найдем его по имени
            $this->Database->query('SELECT id FROM __products_categories WHERE name=?', $item['category']);

            if (!$product['category_id'] = $this->Database->result('id')) {
                // Создадим, если не найден
                $product['category_id'] = $this->import_category($item['category']);
            }
        }
            ...
            foreach ($item as $feature_name => $feature_value) {
                ...
                if (!in_array($feature_name, $this->internal_columns_names)) {
                    ...
                    if ($product['category_id']) {
                        ...
                        $this->ProductsFeatures->addFeatureCategory($feature_id, $product['category_id']);
